...
tax_file = "taxonomy/final/LIB1_target_plant_abundance_taxonomy.tsv"
...
with open(f"{outdir}/LIB1_sample_read_totals.tsv",
          "w", newline="", encoding="utf-8") as f:

    w = csv.writer(f, delimiter="\t")
    w.writerow(["Sample", "Total_target_plant_reads"])
...
for rank in ["Family", "Genus", "Species"]:
    ...
    # Count table
    count_file = (
        f"{outdir}/LIB1_{rank.lower()}_abundance_counts.tsv"
    )
    ...
    rel_file = (
        f"{outdir}/LIB1_{rank.lower()}_relative_abundance.tsv"
    )
    ...
    with open(rel_file, "w", newline="", encoding="utf-8") as f:
        ...
        for sample in sample_totals:
            ...
            for taxon in taxa:
                ...
                vals.append(f"{pct:.6f}")

            w.writerow([sample] + vals)

    # Validation
    classified_reads = 0
    unclassified_reads = 0

    unclassified_name = f"Unclassified_at_{rank.lower()}"

    for sample in sample_taxa:
        for taxon, n in sample_taxa[sample].items():
            if taxon == unclassified_name:
...
print("\n=== LIB1 ECOLOGY INPUT SUMMARY ===")
print("Sample column:", repr(sample_col))
print("Samples:", len(rows))
print("Target ASVs:", len(asv_cols))
print("Total target plant reads:", grand_total)
print("Expected target reads: 360007")
