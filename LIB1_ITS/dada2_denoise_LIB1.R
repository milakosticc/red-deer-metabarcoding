...
errF <- readRDS(
  file.path(err_dir, "LIB1_error_model_R1.rds")
)

errR <- readRDS(
  file.path(err_dir, "LIB1_error_model_R2.rds")
)
...
saveRDS(
  dadaFs,
  file.path(out, "LIB1_dada_R1.rds")
)

saveRDS(
  dadaRs,
  file.path(out, "LIB1_dada_R2.rds")
)
...
write.csv(
  denoise_summary,
  file.path(out, "LIB1_denoising_summary.csv"),
  row.names = FALSE
)
