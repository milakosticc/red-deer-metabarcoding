...
mergers <- readRDS(
  file.path(merged_dir, "LIB2_mergers.rds")
)
...
# Save raw sequence table
saveRDS(
  seqtab,
  file.path(out, "LIB2_raw_ASV_table.rds")
)

write.csv(
  seqtab,
  file.path(out, "LIB2_raw_ASV_table.csv")
)
...
write.csv(
  ...
  file.path(out, "LIB2_ASV_length_distribution.csv"),
  row.names = FALSE
)
