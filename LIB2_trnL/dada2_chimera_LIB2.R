...
# Load raw ASV table
seqtab <- readRDS(
  file.path(asv_dir, "LIB2_raw_ASV_table.rds")
)
...
saveRDS(
  seqtab.nochim,
  file.path(out, "LIB2_nonchim_ASV_table.rds")
)

write.csv(
  seqtab.nochim,
  file.path(out, "LIB2_nonchim_ASV_table.csv")
)
...
write.csv(
  summary,
  file.path(out, "LIB2_chimera_summary.csv"),
  row.names = FALSE
)
