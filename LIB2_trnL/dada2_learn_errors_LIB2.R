...
# Input directory
path <- "/home/mila/aapraksa/samples/working/Plants_trnLc-trnLh/qualFiltered_out"

# Output directory
out <- "/home/mila/aapraksa/samples/working/Plants_trnLc-trnLh/dada2_out/error_models"
...
saveRDS(
  errF,
  file.path(out, "LIB2_error_model_R1.rds")
)

saveRDS(
  errR,
  file.path(out, "LIB2_error_model_R2.rds")
)

# Save error plots
pdf(
  file.path(out, "LIB2_error_rates_R1.pdf"),
  width = 8,
  height = 7
)
...
pdf(
  file.path(out, "LIB2_error_rates_R2.pdf"),
  width = 8,
  height = 7
)
