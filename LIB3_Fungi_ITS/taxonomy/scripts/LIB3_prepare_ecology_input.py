...
# Read final fungal abundance table
...
with open(
    f"{outdir}/LIB3_sample_read_totals.tsv",
    "w", newline="", encoding="utf-8"
) as f:

    w = csv.writer(f, delimiter="\t")
    w.writerow(["Sample", "Total_fungal_reads"])
...
for rank in ["Family", "Genus", "Species"]:
    ...
    with open(
        f"{outdir}/LIB3_{rank.lower()}_abundance_counts.tsv",
        "w", newline="", encoding="utf-8"
    ) as f:
    ...
    with open(
        f"{outdir}/LIB3_{rank.lower()}_relative_abundance.tsv",
        "w", newline="", encoding="utf-8"
    ) as f:
...
print("\n=== LIB3 ECOLOGY INPUT SUMMARY ===")
print("Sample column:", repr(sample_col))
print("Samples:", len(rows))
print("Fungal ASVs:", len(asv_cols))
print("Total fungal reads:", grand_total)
print("Expected fungal reads: 307096")
